...
public class GridLogic : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (PathFinder.instance.debugCurrentIndex > 0)
            {
                --PathFinder.instance.debugCurrentIndex;
                debugGrid();
            }
        }
        if (Input.GetKeyDown("right"))
        {
            if (PathFinder.instance.debugCurrentIndex < PathFinder.instance.debugGridStates.Count - 1)
            {
                ++PathFinder.instance.debugCurrentIndex;
                debugGrid();
            }
    ...
    private void debugGrid()
    {
        for (int i = 0; i < grid.Length; i++)
        {
            ...
            {
                var elem = grid[i][j];
                elem.data = PathFinder.instance.debugGridStates[PathFinder.instance.debugCurrentIndex][i][j];
